...
#define CSI_FRAG_INPUT_BYTES_PER_PIXEL 4U
...
{
    assert(NULL != config);
    assert(NULL != config->dmaBufferAddr);
    assert(NULL != config->dmaBufferAddr[0]);
    assert(NULL != config->dmaBufferAddr[1]);
    assert(NULL != config->dmaBufferAddr[2]);
    assert(NULL != config->dmaBufferAddr[3]);
    uint32_t reg;
    uint32_t instance;
    uint32_t imgWidth_Bytes;
    ...
    handle->dmaBufferAddr[0] = config->dmaBufferAddr[0];
    handle->dmaBufferAddr[1] = config->dmaBufferAddr[1];
    handle->dmaBufferAddr[2] = config->dmaBufferAddr[2];
    handle->dmaBufferAddr[3] = config->dmaBufferAddr[3];
    ...
    CSI_REG_DMASA_FB1(base) = CSI_ADDR_CPU_2_IP((uint32_t)config->dmaBufferAddr[0]);
    CSI_REG_DMASA_FB2(base) = CSI_ADDR_CPU_2_IP((uint32_t)config->dmaBufferAddr[1]);

    if (handle->isDmaBufferCachable)
    {
        DCACHE_CleanInvalidateByRange(
            (uint32_t)config->dmaBufferAddr[0],
            (uint32_t)config->dmaBufferLine * imgWidth_Bytes);
        DCACHE_CleanInvalidateByRange(
            (uint32_t)config->dmaBufferAddr[1],
            (uint32_t)config->dmaBufferLine * imgWidth_Bytes);
        DCACHE_CleanInvalidateByRange(
            (uint32_t)config->dmaBufferAddr[2],
            (uint32_t)config->dmaBufferLine * imgWidth_Bytes);
        DCACHE_CleanInvalidateByRange(
            (uint32_t)config->dmaBufferAddr[3],
            (uint32_t)config->dmaBufferLine * imgWidth_Bytes);
    }
    
    if (4U == CSI_FRAG_INPUT_BYTES_PER_PIXEL)
    {
        CSI_REG_CR18(base) |= CSI_CR18_PARALLEL24_EN_MASK;
    }
                                          ...
                                          const csi_frag_capture_config_t *config)
{
    ...
    /* Image parameter. */
    CSI_REG_IMAG_PARA(base) =
        (((uint32_t)handle->width) << CSI_IMAG_PARA_IMAGE_WIDTH_SHIFT) |
...
void CSI_FragModeTransferHandleIRQ(CSI_Type *base, csi_frag_handle_t *handle)
{
    ...
    if (0U != (csisr & (uint32_t)kCSI_StartOfFrameFlag))
    {
        ...
        if (NULL != handle->callback)
        {
            handle->callback(base, handle, kStatus_CSI_FrameStart, handle->userData);
        }
    }
    ...
    if ((csisr & (CSI_SR_DMA_TSF_DONE_FB2_MASK | CSI_SR_DMA_TSF_DONE_FB1_MASK)) ==
        (CSI_SR_DMA_TSF_DONE_FB2_MASK | CSI_SR_DMA_TSF_DONE_FB1_MASK))
    {
        frameValid = false; /* Skip the frames. */
    }

    if(frameValid && (csisr & (CSI_SR_DMA_TSF_DONE_FB2_MASK | CSI_SR_DMA_TSF_DONE_FB1_MASK)))
    {
        if (csisr & CSI_SR_DMA_TSF_DONE_FB1_MASK)
        {
            dmaBufAddr = CSI_REG_DMASA_FB1(base);
            if(dmaBufAddr == (uint32_t)handle->dmaBufferAddr[0])
            {
                CSI_REG_DMASA_FB1(base) = (uint32_t)handle->dmaBufferAddr[2];
            }
            else
            {
                CSI_REG_DMASA_FB1(base) = (uint32_t)handle->dmaBufferAddr[0];
            }
        }
        else
        {
            dmaBufAddr = CSI_REG_DMASA_FB2(base);
            if(dmaBufAddr == (uint32_t)handle->dmaBufferAddr[1])
            {
                CSI_REG_DMASA_FB2(base) = (uint32_t)handle->dmaBufferAddr[3];
            }
            else
            {
                CSI_REG_DMASA_FB2(base) = (uint32_t)handle->dmaBufferAddr[1];
            }
        }
        ...
        if (NULL != handle->callback)
        {
            handle->callback(base, handle, kStatus_CSI_FragBufferFull, handle->userData);
        }
