...
enum
{
    ...
    kStatus_CSI_FrameStart = MAKE_STATUS(kStatusGroup_CSI, 4),
    kStatus_CSI_FragBufferFull = MAKE_STATUS(kStatusGroup_CSI, 5),
};
...
#define CSI_FRAG_BUFFER_COUNT 4
...
typedef struct _csi_frag_config
{
    ...
    uint8_t * dmaBufferAddr[CSI_FRAG_BUFFER_COUNT];/*!< Buffer addresses used for CSI DMA, must be double word aligned. */
